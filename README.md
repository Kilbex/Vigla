...
Deploy Claude Code, Codex CLI, Antigravity, and other agent CLIs as one supervised team —
each agent in its own git worktree, every merge audited, every mission
revertable in one click.
...
![A recorded Vigla mission replay: cross-vendor AI coding agents execute, pause, and pass through audit outcomes in the Operations Room](docs/media/vigla-demo.webp)
...
- **Local-first.** A Rust orchestrator, Tauri shell, and SQLite event
  store — no cloud control plane, no account. The only network traffic
  is what your vendor CLIs already send.
...
[ARCHITECTURE.md](./ARCHITECTURE.md). Newcomer-friendly tasks live in

...
//! on the child's exit status. This is the stable fallback contract for
//! a CLI that does not expose the canonical Vigla event schema.
...
impl KiroAdapter {
    pub fn new(worker_id: impl Into<String>, task_id: Option<String>) -> Self {
        Self {
            inner: adapter_core::RawLogAdapter::new(worker_id, task_id, "kiro worker finished"),
        }
    }
}

impl adapter_core::Adapter for KiroAdapter {
    fn ingest_line(
        &mut self,
        line: &str,
...
mod tests {
    ...
    fn quota_line_surfaces_signal_through_wrapper() {
        let mut a = KiroAdapter::new("w1", None);
